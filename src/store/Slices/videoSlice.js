...
export const getAllVideos = createAsyncThunk(
    ...
    async ({ userId, sortBy, sortType, query, page, limit }) => {
        try {
            ...
            // Filter videos on the client side based on the query
            if (query) {
                filteredResp = filteredResp.filter(video =>
...
// new added
export const getUserVideos = createAsyncThunk("getUserVideos", async (userId) => {
    try {
...
const videoSlice = createSlice({
    ...
    reducers: {
        ...
        makeVideosNull: (state) => {
            state.videos = { docs: [], hasNextPage: false };
        },
    },
    extraReducers: (builder) => {
        builder.addCase(getAllVideos.pending, (state) => {
        ...
        builder.addCase(getAllVideos.fulfilled, (state, action) => {
            state.loading = false;
            // Clear existing videos when a new search is performed
        ...
        builder.addCase(getUserVideos.fulfilled, (state, action) => {
            state.loading = false;
            state.ownVideos.docs = action.payload;
        });
    },
});
